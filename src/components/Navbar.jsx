    ...
    <nav className={`${styles.paddingX} w-full flex items-center py-5 fixed top-0 z-20 bg-primary `}>
      <div className="flex w-full justify-between items-center max-w-7xl mx-auto">
        ...
        >
          ...
          <p className="text-white text-[18px] font-semibold cursor-pointer">Shabbir Hossain Shovo</p>
        </Link>
        ...
        <div className="sm:hidden flex flex-1 justify-end items-center">
          ...
          <ul className="list-none flex flex-col gap-2">
          {
            navLinks.map((link) => (
              <li key={link.id}
                className={`${active === link.title ? "text-white" : " text-secondary "} hover:text-white font-medium text-[18px] font-poppins cursor-pointer`}
                onClick={() => 
                  {setToggle(!toggle)
                  setActive(link.title)}}
              >
